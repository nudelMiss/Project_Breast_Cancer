...
meta.to_csv(OUT/"umap_emb_6cohort_v2_coords.csv",index=False)
...
fig.savefig(OUT/"umap_emb_6cohort_v2_ellipse.png",dpi=160,bbox_inches='tight',bbox_extra_artists=(leg1,leg2))
plt.close(fig)
print("[FIG] scripts/visualization/umap/umap_emb_6cohort_v2_ellipse.png",flush=True); print("DONE",flush=True)

# --- no-ellipse variant ---
fig2,axes2=plt.subplots(1,2,figsize=(20,8))
for ax,coords,ttl in [(axes2[0],co_raw,"Pre-correction (raw profiles)"),(axes2[1],co_bc,"Within-cohort centered (batch-corrected)")]:
    for i,r in enumerate(recs):
        ax.scatter(coords[i,0],coords[i,1],c=r['color'],marker=MK[r['ds']],s=58,alpha=0.82,edgecolors='white',linewidths=0.4,zorder=3)
    ax.set_xlabel("UMAP 1"); ax.set_ylabel("UMAP 2"); ax.set_title(ttl)
seen2={}
for r in recs: seen2[r['broad']]=r['color']
g1=[Line2D([0],[0],marker='o',color='w',markerfacecolor=c,markersize=11,label=l) for l,c in seen2.items()]
g2=[Line2D([0],[0],marker=MK[d],color='w',markerfacecolor='grey',markeredgecolor='k',markersize=10,label=DSLAB[d]) for d in DATASETS]
lg1=axes2[1].legend(handles=g1,title="Cell type",loc='upper left',bbox_to_anchor=(1.02,1.0),fontsize=10); axes2[1].add_artist(lg1)
lg2=axes2[1].legend(handles=g2,title="Cohort",loc='upper left',bbox_to_anchor=(1.02,1.0-0.075*(len(g1)+1.8)),fontsize=10)
fig2.suptitle(f"UMAP per-patient propr embeddings (no ellipse) -- {len(recs)} groups, 6 cohorts, anchor={len(anchor)} genes",fontsize=13)
fig2.savefig(OUT/"umap_emb_6cohort_v2_noellipse.png",dpi=160,bbox_inches='tight',bbox_extra_artists=(lg1,lg2)); plt.close(fig2)
print("[FIG] scripts/visualization/umap/umap_emb_6cohort_v2_noellipse.png",flush=True)
