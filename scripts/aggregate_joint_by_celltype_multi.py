#!/usr/bin/env python3
"""Generic joint-by-cell-type embeddings (parametrized models_root/out_root).
Pools all per-patient groups of one cell type (chained edges.tsv walk-corpora)
into a single Word2Vec. Mirrors scripts/bassez2021/aggregate_joint_by_celltype.py."""
...
def main():
    ...
    ap.add_argument("--sim", required=True, choices=["propr","cosine","spearman","npmi"])
    ap.add_argument("--strat", required=True, choices=["star","bidirectional"])
    args = ap.parse_args()
    PPMODELS = Path(args.models_root); OUTROOT = Path(args.out_root)
    ...
    edge_paths = [gd / cfg_tag / "edges.tsv" for gd in grp_dirs if (gd / cfg_tag / "edges.tsv").exists()]
    if not edge_paths:
        sys.exit(f"FATAL: no per-patient edges for celltype={args.celltype} cfg={cfg_tag} under {PPMODELS}")
    print(f"[JOINT-CT] {args.celltype} {cfg_tag}: pooling {len(edge_paths)} groups", flush=True)
    ...
    with open(out_dir / "run_config.json", "w") as f:
        json.dump(dict(celltype=args.celltype, sim=args.sim, strat=args.strat, walks=W, k=K,
                       walk_length=WL, hvg_cap=2000, aggregation="joint_by_celltype",
                       n_groups_pooled=len(edge_paths)), f, indent=2)
    print(f"[SAVE] {model_path} (pooled {len(edge_paths)})", flush=True)
