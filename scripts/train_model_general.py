...
def run_one_group(group_dir: Path, out_root: Path, cfg: RunConfig, *, save_edges: bool, skip_w2v: bool):
    ...
    # optionally stop here (useful for AUC benchmark stage)
    if skip_w2v:
        print("skip_w2v=True => stopping after graph/edges export.", flush=True)
        return
    ...
    print(f"SLURM_CPUS={slurm_cpus} W2V_WORKERS={w2v_workers} materialize={cfg.materialize}", flush=True)
    ...
    if cfg.materialize:
        print("Materializing walks into RAM ...", flush=True)
        walks = list(corpus)
...
def parse_args():
    ...
    p.add_argument("--out_root", type=str, default="results_by_patient_celltype")
