...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Run only a specific group folder name under in_root (exact folder name).",
)
...
# Use SLURM cpus-per-task if available, else fallback
...
def run_one_group(group_dir: Path, out_dir: Path):
    ...
    # IMPORTANT: since clamping removed, enforce non-negative weights
    if w_min < 0:
        raise ValueError(
    ...
    # Word2Vec
    print(f"Training Word2Vec model... (workers={W2V_WORKERS})")
    model = Word2Vec(
...
def main():
    ...
    if args.only_group is not None and len(group_dirs) == 0:
        raise FileNotFoundError(f"only_group not found under in_root: {args.only_group}")
