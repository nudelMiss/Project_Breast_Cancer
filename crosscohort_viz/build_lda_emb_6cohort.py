...
make_lda(True ,"lda_emb_6cohort_ellipse.png")
make_lda(False,"lda_emb_6cohort_noellipse.png")
pd.DataFrame({'ds':dsl,'ct':[r['ct'] for r in recs],'broad':list(broad),
  'LD1_raw':ld_raw[:,0],'LD2_raw':ld_raw[:,1],'LD1_bc':ld_bc[:,0],'LD2_bc':ld_bc[:,1]}).to_csv(OUT/"lda_emb_6cohort_coords.csv",index=False)
